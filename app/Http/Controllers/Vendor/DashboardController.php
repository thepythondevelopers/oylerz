...
{
    public $path ='vendor.modules.';
    
    public function index()
    {
        $d = date('d/m/Y');
        $booking_count = Booking::where('vendor',Auth::user()->id)->count();
        $booking_sum = Booking::where('vendor',Auth::user()->id)->sum('price');
        $booking_completed = Booking::where('vendor',Auth::user()->id)->where('status',1)->count();
        $booking_today = Booking::where('vendor',Auth::user()->id)->where('date',$d)->count();
    ...
    public function get_chart(Request $request){
        ...
        $data = Booking::where('vendor',Auth::user()->id)->where( 'year', $year )->get()->groupBy(function($val) {
        ...
        $complete_data = Booking::where('vendor',Auth::user()->id)->where( 'year', $year )->where('status',1)->get()->groupBy(function($val) {
            return Carbon::parse(strtotime($val->date))->format('M');
        });
