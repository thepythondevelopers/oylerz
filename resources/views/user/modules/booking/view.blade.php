        ...
        <div class="csm-card">
            ...
            <div class="col-lg-6">
                    ...
                    @if($b->status !=1)    
                       ...
                       @if(strtotime(\Carbon\Carbon::now(user_location()->timezone)) > strtotime(\Carbon\Carbon::createFromFormat('d/m/Y h:i a', $b->date.' '.$b->time_slot)->format('d-m-Y h:i a')))
                       <label></label>
                       @else
                       @if( round((strtotime(\Carbon\Carbon::createFromFormat('d/m/Y h:i a', $b->date.' '.$b->time_slot)->format('d-m-Y h:i a')) -strtotime(\Carbon\Carbon::now(user_location()->timezone)) )/3600, 1) > 1) 
                        <a href="{{route('user.booking.reschedule',$b->id)}}" class="main-btn">Reschedule</a> 
                        @endif
                       @endif
                    @endif   
                    ...
                    <div class="btn-wrap mt-4">
                        <a href="{{route('user.booking')}}" class="main-btn">Back</a>
                    </div>
                </div>
        </div> 
        </div>
